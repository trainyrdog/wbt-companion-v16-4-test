...
// WBT Companion App v16.7 — Pre-Test QA Polish
// Pre-test QA polish after deeper route checks: fixes Dog Profile runtime issue, report dog-age note, compact public screens, and safer pre-test wording.
...
const APP_META = {
  name: "WBT Companion",
  version: "v16.7",
  build: "Pre-test QA polish candidate",
  publicLabel: "WBT Companion",
  storageScope: "This device only",
  installReadyNote:
    "Prepared for real phone/browser testing as a clean local WBT Companion. Saved information remains local to the current device/browser in this version. v16.7 focuses on public wording, compact screens, safer navigation, and fixed pre-test route issues.",
};
...
function Home({ setScreen }) {
  ...
  return (
    <div>
      <Hdr title="WBT Companion" sub="Create the dog profile, run one assessment, read the report, and follow the next WBT route." />
      <PageBody style={{ gap: 10 }}>
        ...
        <Card tone="raised" style={{ padding: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, marginBottom: 8 }}>
            <MiniLabel>Next actions</MiniLabel>
            <Pill col={completion.ready ? B.grn : B.amber}>{completion.ready ? "Ready" : "Profile first"}</Pill>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <Button active={!completion.ready} onClick={() => setScreen("profile")} style={{ width: "100%" }}>Dog Profile</Button>
            <Button active={completion.ready && !activeObservation} onClick={() => setScreen("behaviour")} style={{ width: "100%" }}>Assessment</Button>
            <Button active={Boolean(activeObservation)} onClick={() => setScreen("log")} style={{ width: "100%" }}>Observation</Button>
            <Button onClick={() => setScreen("assistant")} style={{ width: "100%" }}>WBT Guide</Button>
          </div>
          <SmallText>{completion.ready ? "The profile is ready enough for a stronger report." : `${completion.done}/${completion.total} key profile fields complete. Add a little more context before the assessment.`}</SmallText>
        </Card>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          <HomeTile icon="📚" title="Learn" text="Open WBT education routes." onClick={() => setScreen("library")} />
          <HomeTile icon="🚨" title="Help" text="Safety-first guidance." onClick={() => setScreen("emergency")} />
        </div>
...
function DeviceStorageNotice() {
  return (
    <Card style={{ padding: 11, background: "rgba(255,248,238,.035)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        <div>
          <MiniLabel>Saved on this device</MiniLabel>
          <SmallText>Profile, reports, and observations stay in this browser. Use the same device/browser to continue.</SmallText>
        </div>
        <Pill col={B.grn}>Local</Pill>
      </div>
    </Card>
  );
}
...
function HomeTile({ icon, title, text, onClick }) {
  return (
    <div onClick={onClick} style={{ background: gradient.card, border: UI.cardBorder, borderRadius: UI.radius.lg, padding: 13, cursor: "pointer", boxShadow: "0 8px 18px rgba(0,0,0,.12)", minHeight: 92, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative" }}>
        ...
        <div style={{ color: B.white, fontWeight: 950, fontSize: 13, marginTop: 8, letterSpacing: -0.1 }}>{title}</div>
        <div style={{ color: B.sand2, fontSize: 10.5, lineHeight: 1.35, marginTop: 4 }}>{text}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <Card style={{ borderColor: col, background: `radial-gradient(circle at top right, ${col}30, transparent 42%), linear-gradient(145deg, ${B.card2}, ${B.bg})`, boxShadow: UI.shadow }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 13 }}>
        <ReportMiniStat label="Dog" value={dogName(report.profile)} color={B.accLt} note={ageText(report.profile)} />
...
class AppErrorBoundary extends Component {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div>
          <Hdr title="Screen reset" sub="This screen did not load correctly. Your saved local data has not been cleared." />
          <PageBody>
            <Card tone="raised" style={{ borderColor: "rgba(225,154,99,.38)" }}>
              <MiniLabel>Safe fallback</MiniLabel>
              <h2 style={{ color: B.white, fontSize: 18, margin: "6px 0" }}>Return to a stable screen</h2>
              <SmallText>Return to a stable screen and try again. If it repeats, note which button you pressed before it happened.</SmallText>
              <div style={{ display: "grid", gap: 8, marginTop: 12 }}>
                <Button active onClick={() => this.props.setScreen("home")}>Back to dashboard</Button>
                <Button onClick={() => this.props.setScreen("profile")}>Open Dog Profile</Button>
              </div>
            </Card>
          </PageBody>
        </div>
...
export default function WBTCompanionApp() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100dvh", minHeight: "100vh", background: gradient.app, color: B.white, fontFamily: UI.font, overflow: "hidden" }}>
      <div ref={contentRef} style={{ flex: 1, overflowY: "auto", maxWidth: UI.maxW, width: "100%", margin: "0 auto", background: "rgba(15,23,18,.96)", boxShadow: UI.shadow, borderLeft: UI.cardBorder, borderRight: UI.cardBorder }}>
        <AppErrorBoundary screen={screen} setScreen={setScreen}>{content}</AppErrorBoundary>
      </div>
